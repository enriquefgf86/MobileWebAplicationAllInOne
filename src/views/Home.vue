<template>
  <div v-if="user" class="home">
    ...
    <SearchBar @receivingSelfSearch="autoSearch" v-bind:val="Search"></SearchBar>
    <!-- <filter-tag ></filter-tag> -->
    <Filters
      @receiveCountryList="theCountryList"  v-bind:countryList="allCountries"
      @receiveEventList="theEventList"    v-bind:eventList="allEvents"
      @receiveModalityList="theModalityList"   v-bind:modalityList="allModalities"
    ></Filters>
    <div class="container1">
      <v-content id="containerScroll">
        <v-container v-if="loadingLoader">
          <v-flex xs12 class="primary--text">
            <v-progress-circular indeterminate :width="7" :size="50" color="primary"></v-progress-circular>
          </v-flex>
        </v-container>
    ...
    autoSearchFilter: function() {
      ...
      if (this.Search != 0) {
        return this.getAllEvents.filter(selfSearch => {
          return (
            selfSearch.classifications[0].genre.name.toUpperCase().includes(this.Search.toUpperCase()) ||
            selfSearch.classifications[0].subGenre.name.toUpperCase().includes(this.Search.toUpperCase()) ||
      ...
      return Array.from(
        new Set(
          this.getAllEvents
            .filter(
              eventSelected =>
                eventSelected.classifications[0].segment.name == this.listEvents
            )
